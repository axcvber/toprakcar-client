const ru = {
  ...
  validation: {
    ...
    search: {
      ...
      dropOffLocation: {
        required: 'Выбирете место возврата',
      },
      pickUpDate: {
  ...
  dataState: {
    error: {
      ...
      subtitle:
        'Приготовьтесь, пока мы не исправим ошибку. Вы также можете обновить страницу или повторить попытку позже',
    },
    notFound: {
      title: 'Результаты не найдены',
      subtitle: 'Попробуйте удалить некоторые фильтры или увеличить диапазон, чтобы увидеть больше результатов',
    },
  },
  cookiesConsent: {
    text: `Мы используем файлы cookie для улучшения вашего опыта просмотра, предоставления персонализированной рекламы или контента, а также для анализа нашего трафик. Нажав кнопку "Принять все", вы соглашаетесь с использованием нами файлов cookie.`,
  },
}
