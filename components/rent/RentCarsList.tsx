...
const RentCarsList: React.FC<IRentCarsList> = ({ withLocationChange }) => {
  const { filtered, sortBy } = useFilterContext()
  const { pickUpLocation, pickUpDate, dropOffDate, setCurrentStep, setSelectedCar } = useRentContext()
  ...
  return (
    <Grid container spacing={3} mb={2}>
      ...
      <Grid item xs={12} lg={9}>
        ...
        <DataList isError={!!error} isEmpty={data?.rentCars?.data?.length === 0}>
          <>
            {loading ? (
              <Loader quantity={6}>
